...
class MyImageboardPage extends Component {
 componentDidMount() {
  ...
  render() {
    ...
    return (
      <div>
        ...
        <button className="btn btn-dark" onClick={this.onLogout}>Logout</button>
        <CreateImageFormContainer/>
...
function mapStateToProps(state) {
  return { userImages: state.users.uniqueUser.images,
    userName: state.users.uniqueUser.username, userLoggedIn: state.userLoggedIn};
}
const mapDispatchToProps = { getUser, logout };

export default connect(mapStateToProps, mapDispatchToProps)(MyImageboardPage);